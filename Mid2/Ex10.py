...
if __name__ == '__main__':
    ...
    train_set = dataset[:int(0.80 * len(dataset))]
    ...
    ver_set = dataset[int(0.80 * len(dataset)):]
    ver_x = [row[:-1] for row in ver_set]
    ver_y = [row[-1] for row in ver_set]
    ver_x = scalar.transform(ver_x)

    neurons = int(input())
    learn_rate = float(input())
    col = int(input())

    classifier = MLPClassifier(hidden_layer_sizes=neurons, activation="relu", learning_rate_init=learn_rate,
    ...
    ver_acc = 0
    for i in range(len(ver_set)):
        if classifier.predict([ver_x[i]])[0] == ver_y[i]:
            ver_acc += 1
    ver_acc = ver_acc / len(ver_set)

    inp = [float(x) for x in input().split(' ')]

    if ver_acc / 1.15 < train_acc:
        print("Se sluchuva overfitting")
        dataset = [row[:col] + row[col+1:] for row in dataset]
        train_set = dataset[:int(0.80 * len(dataset))]
        train_x = [row[:-1] for row in train_set]
        train_y = [row[-1] for row in train_set]
        scalar = MinMaxScaler(feature_range=(-1, 1))
