...
class Snake(Problem):
    ...
    # Enter snake and the new pos of the head
    def check_valid(self, snake, head):
        check = (head not in snake[1:] and head not in self.redApples and \
            self.box[0] > head[0] >= 0 and self.box[0] > head[1] >= 0)
        return head not in snake[1:] and head not in self.redApples and \
            self.box[0] > head[0] >= 0 and self.box[0] > head[1] >= 0

    def move(self, state, direction):
        snake = state[0]
        orientation = (snake[-1][0] - snake[-2][0], snake[-1][1] - snake[-2][1])
        if direction == "s":
            if orientation[0] != 0:
                return snake[-1][0] + orientation[0], snake[-1][1]
            else:
                return snake[-1][0], snake[-1][1] + orientation[1]
        elif direction == "l":
            if orientation[0] != 0:
                return snake[-1][0], snake[-1][1] + orientation[0]
            else:
                return snake[-1][0] - orientation[1], snake[-1][1]
        elif direction == "r":
            if orientation[0] != 0:
                return snake[-1][0], snake[-1][1] - orientation[0]
            else:
    ...
    def move_get_state(self, state, direction):
        snake = list(state[0])
        greenApples = list(state[1])
        snake.append(self.move(state, direction))
        if snake[-1] in greenApples:
            greenApples.remove(snake[-1])
            snake = tuple(snake)
            greenApples = tuple(greenApples)
        else:
            snake.pop(0)
            snake = tuple(snake)
            greenApples = tuple(greenApples)

        return (snake, greenApples)
...
if __name__ == '__main__':
    numberOfGreen = int(input())
    greenApples = list()
    for i in range(numberOfGreen):
        inp = input()
        greenApples.append((int(inp.split(",")[0]), int(inp.split(",")[1])))

    numberOfRed = int(input())
    redApples = list()
    for i in range(numberOfRed):
        inp = input()
        redApples.append((int(inp.split(",")[0]), int(inp.split(",")[1])))

    snake = Snake((((0, 9), (0, 8), (0, 7)), tuple(greenApples), tuple(redApples)))
    snake = breadth_first_graph_search(snake)
